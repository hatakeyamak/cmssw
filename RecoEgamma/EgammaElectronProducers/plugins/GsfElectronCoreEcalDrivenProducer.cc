...
void GsfElectronCoreEcalDrivenProducer::produce(edm::StreamID, edm::Event& event, const edm::EventSetup&) const {
  ...
  auto produceEcalDrivenCore = [&](const reco::GsfTrackRef& gsfTrackRef) {
    ...
    std::cout << scRef->seed()->caloID().detector(reco::CaloID::DET_ECAL_BARREL) << " "
              << scRef->seed()->caloID().detector(reco::CaloID::DET_ECAL_ENDCAP) << " "
              << scRef->seed()->caloID().detector(reco::CaloID::DET_HGCAL_ENDCAP) << " " << useGsfPfRecTracks_
              << std::endl;
  };
