...
PFHBHERecHitProducerCPU::PFHBHERecHitProducerCPU(edm::ParameterSet const& ps)
    : produceSoA_{ps.getParameter<bool>("produceSoA")},
      ...
      OutputPFRecHitSoA_Token_{produceSoA_
                                   ? produces<OProductType>(ps.getParameter<std::string>("PFRecHitsSoALabelOut"))
                                   : edm::EDPutTokenT<OProductType>{}},  // empty token if disabled
      OutputPFRecHitLegacy_Token_{
          produceLegacy_ ? produces<reco::PFRecHitCollection>(ps.getParameter<std::string>("PFRecHitsLegacyLabelOut"))
                         : edm::EDPutTokenT<reco::PFRecHitCollection>{}},  // empty token if disabled
      OutputPFRecHitCleanedLegacy_Token_{
          produceCleanedLegacy_
