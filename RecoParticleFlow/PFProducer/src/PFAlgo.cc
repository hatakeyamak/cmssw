...
void PFAlgo::processBlock( const reco::PFBlockRef& blockref,
  ...
  typedef std::multimap<double, std::tuple<unsigned,::math::XYZVector,double> >::iterator IS;
...
//       cerr<<"not ready for navigation in the HF!"<<endl;
  ...
  for(unsigned i=0; i<hcalIs.size(); i++) {
    ...
    for ( IS is = ecalSatellites.begin(); is != ecalSatellites.end(); ++is ) { 
      ...
      //
      totalEcal         += sqrt(std::get<1>(is->second).Mag2()); // KH: raw ECAL energy for input to PF hadron calibration
      totalEcalEGMCalib += sqrt(std::get<1>(is->second).Mag2())*std::get<2>(is->second); // KH: calibrated ECAL energy under the egamma hypothesis
      photonAtECAL      += std::get<1>(is->second)*std::get<2>(is->second); // KH: calibrated ECAL energy under the egamma hypothesis
	...
	if(debug_) cout<<"\t\t\tactive, adding "<<std::get<1>(is->second)
		       <<" to ECAL energy, and locking"<<endl;
	active[std::get<0>(is->second)] = false;
        double clusterEnergy=sqrt(std::get<1>(is->second).Mag2())*std::get<2>(is->second); // KH: ECAL energy calibrated under the egamma hypothesis
      ...
      totalEcal -= sqrt(std::get<1>(is->second).Mag2());
      totalEcalEGMCalib -= sqrt(std::get<1>(is->second).Mag2())*std::get<2>(is->second);
      photonAtECAL -= std::get<1>(is->second)*std::get<2>(is->second);
      calibEcal = previousCalibEcal;
    ...
    */
    ...
    for ( IS is = ecalSatellites.begin(); is != ecalSatellites.end(); ++is ) { 
      ...
      unsigned iEcal = std::get<0>(is->second);
      if ( !active[iEcal] ) continue;
      ...
      // Create a photon
      double ecalClusterEnergyCalibrated = sqrt(std::get<1>(is->second).Mag2()) * std::get<2>(is->second); // KH: calibrated under the egamma hypothesis (rawEcalClusterEnergy * calibration)
      unsigned tmpi = reconstructCluster( *eclusterref, ecalClusterEnergyCalibrated ); 
      (*pfCandidates_)[tmpi].setEcalEnergy( eclusterref->energy(),ecalClusterEnergyCalibrated );
      (*pfCandidates_)[tmpi].setHcalEnergy( 0., 0. );
      (*pfCandidates_)[tmpi].setHoEnergy( 0., 0. );
      ...
      if ( fabs(eclusterref->energy() - sqrt(std::get<1>(is->second).Mag2()))>1e-3 || fabs(eclusterref->correctedEnergy() - ecalClusterEnergyCalibrated)>1e-3 )
	edm::LogWarning("PFAlgo:processBlock") << "ecalCluster vs ecalSatellites look inconsistent (eCluster E, calibE, ecalSatellite E, calib E): "
					       ...
					       << sqrt(std::get<1>(is->second).Mag2()) << " " << ecalClusterEnergyCalibrated;
