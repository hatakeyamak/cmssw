...
int CaloTowersDQMClient::CaloTowersEndjob(const std::vector<MonitorElement*> &hcalMEs){
   ...
   for (int i = 1; i <= nx; i++) {
      ...
      int ieta = i - 43; //should be the same as int ieta = int(occupancy_vs_ieta->getTH1F()->GetBinCenter(i));
      std::cout << ieta << " " << int(occupancy_vs_ieta->getTH1F()->GetBinCenter(i)) << std::endl;
